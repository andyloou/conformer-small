...
class ConformerCTC(nn.Module):
    def __init__(self, vocab_path: str, **model_config):
        """
        Khởi tạo mô hình ConformerCTC.
        
        Args:
            vocab_path (str): Đường dẫn đến file bpe.vocab.
            **model_config: Các tham số cấu hình mô hình khác 
                              (ví dụ: encoder_params, num_mel_bins, ...).
        """
        super().__init__()

        # --- Tải Vocabulary ---
        logger.info(f"Đang tải vocabulary từ: {vocab_path}")
        self.vocabulary = load_vocabulary(vocab_path)
        self.vocab_size = len(self.vocabulary)
        
        # Tự động tìm pad_id (giả định token là '<pad>')
        try:
            self.pad_id = self.vocabulary.index('<pad>')
        except ValueError:
            logger.warning("Không tìm thấy token '<pad>' trong vocabulary. Giả định pad_id=0.")
            self.pad_id = 0  # Giả định mặc định
        
        # Blank ID luôn là chỉ số cuối cùng (sau khi vocab đã được load)
        self.blank_id = self.vocab_size
        logger.info(f"Đã tải vocabulary: {self.vocab_size} tokens. pad_id={self.pad_id}, blank_id={self.blank_id}")
        # --- Kết thúc tải Vocabulary ---
        ...
        self.decoder = ConvASRDecoder(
            feat_in=d_model,
            num_classes=-1, # Sẽ được ghi đè bởi vocabulary
            vocabulary=self.vocabulary, # Truyền vocab list đã tải
            add_blank=True,
        )
    ...
    def forward(self, mel_feats, mel_lens, targets=None, target_lens=None):
        ...
        if targets is not None and target_lens is not None:
            # Chuyển (B, T, C) -> (T, B, C) cho CTCLoss
            ...
            output.update({
                "loss": loss,
                "ctc_loss": loss,
                "decoder_loss": torch.tensor(0.0, device=loss.device), # Giữ tương thích
            })
...
if __name__ == "__main__":
    VOCAB_PATH = "/home/andyloou/bud500/viet-asr/data/bpe_4000/bpe.vocab"
    
    model = ConformerCTC(vocab_path=VOCAB_PATH)

    checkpoint_path = "/home/andyloou/bud500/viet-asr/vietasr/epoch_37.pt"
    ...
    max_audio_length = 3000  # Adjust based on your max audio length
    model.encoder.set_max_audio_length(max_audio_length)
    print(f"✅ Pre-extended positional encoding to {max_audio_length}")
    try:
        class EncoderDecoderWrapper(nn.Module):
            ...
            def forward(self, mel_spectrogram, mel_length):
                encoded, encoded_length = self.encoder(mel_spectrogram, mel_length)
                log_probs = self.decoder(encoded)
                return log_probs, encoded_length

        wrapper = EncoderDecoderWrapper(model.encoder, model.decoder)
        wrapper.eval()
        wrapper.to(device) # Đảm bảo wrapper cũng trên device
        batch_size = 1
        mel_features = 80
        time_steps = 100
        dummy_mel_input = torch.randn(batch_size, mel_features, time_steps).to(device)
        dummy_mel_length = torch.tensor([time_steps], dtype=torch.int64).to(device)
        try:
            print("Testing JIT tracing...")
            traced = torch.jit.trace(wrapper, (dummy_mel_input, dummy_mel_length))
            traced_out = traced(dummy_mel_input, dummy_mel_length)
            print("✅ JIT tracing successful")
        except Exception as e:
            print(f"⚠️ Tracing warning: {e}")
        # Create dummy mel spectrogram input
      

        torch.onnx.export(
            wrapper,
            (dummy_mel_input, dummy_mel_length),
            output_onnx_path,
            export_params=True,
            opset_version=17,
            do_constant_folding=True,
            input_names=['mel_spectrogram', 'mel_length'],
            output_names=['log_probs', 'encoded_length'],
            dynamic_axes={
                'mel_spectrogram': {0: 'batch_size', 2: 'time_steps'},
                'mel_length': {0: 'batch_size'},
                'log_probs': {0: 'batch_size', 1: 'output_time_steps'},
                'encoded_length': {0: 'batch_size'}
            }
        )
        print(f"Successfully exported encoder + decoder to ONNX: {output_onnx_path}")
        print(f"Input: mel_spectrogram [batch, {mel_features}, time], mel_length [batch]")
        # In ra số class (vocab_size + 1 blank)
        print(f"Output: log_probs [batch, time, num_classes={model.vocab_size + 1}], encoded_length [batch]") 
    ...
    print(f"\nFull model test (với dummy mel spectrogram):")
    with torch.no_grad():
        output_dict = model(dummy_mel_input, dummy_mel_length)
        log_probs = output_dict["log_probs"]
        encoded_length = output_dict["encoder_out_lens"]
        
    print(f"Output log_probs shape: {log_probs.shape}")
    print(f"Output encoded_length: {encoded_length}")
    ...
    with torch.no_grad():
        pt_outputs = wrapper(dummy_mel_input, dummy_mel_length)
    
    max_diff = np.abs(ort_outputs[0] - pt_outputs[0].cpu().numpy()).max()
